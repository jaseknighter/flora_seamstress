...
local function new_arb_graph(x_min, x_max, y_min, y_max)
  ...
  -- local graph = Graph.new(x_min, x_max, "lin", y_min, y_max, "lin", "spline", false, false)
  -- local graph = Graph.new(x_min, x_max, "lin", y_min, y_max, "lin", "spline_and_point", false, false)
...
function ArbGraph:draw_bars(h_bar_percentage, v_bar_percentage)
  ...
  screen.level(3)
  screen.move(self:get_x()-7, self:get_y()+self:get_height()+8)
  ...
  if self:get_x() < 20 then
    screen.move(self:get_x()-6, self:get_y()+6)
  else
    screen.move(self:get_x()-6, self:get_y()+6)
  end
  screen.line_rel(3,0)
  ...
  screen.line_rel(self:get_width()+2,0)
  --screen.stroke()
  screen.move(self:get_x() -1 + self:get_width(), self:get_y()+self:get_height()+6)
  screen.line_rel(0,3)
  --screen.stroke()
  screen.move(self:get_x() - 1 + self:get_width(), self:get_y()+self:get_height()+10)
  screen.line_rel(-self:get_width()-6,0)
  --screen.stroke()
end
