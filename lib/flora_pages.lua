...
local draw_plants = function()
  if pages.index < 4 then
    screen.move(0,13)
    -- screen.level(0)
    screen.rect_fill(128,51)
    --screen.stroke()
    screen.level(plant1_screen_level+5)
    plants[1].redraw_fn()
    screen.level(plant2_screen_level+5)
    plants[2].redraw_fn()
  end  
end
...
local draw_notes_on_plants = function()
  ...
  if plants[1].show_note and env_level1 > 0.1 then
    ...
    if prev_turtle_pos and prev_turtle_pos.x  then
      ...
      -- screen.circle(2)
      screen.pixel(prev_turtle_pos.x, prev_turtle_pos.y)
      --screen.stroke()
      -- --screen.update()
    ...
    if turtle_pos and turtle_pos.x  then
      ...
      screen.pixel(turtle_pos.x, turtle_pos.y)
      -- screen.circle(2)
      --screen.stroke()
    end 
  end
...
local draw_top_nav = function()
  if nav_off == false then
    ...
    --screen.stroke()
    screen.move(0,0)
    screen.rect_fill(screen_size.x,10)
    screen.level(0)
  end
  -- screen.move(4,7)
  screen.move(4,1)
  ...
  elseif pages.index == 4 then
    ...
    elseif graph_active_node == 0 then 
      ...
      elseif envelopes[active_plant].active_node_param == 3 then
        ...
        local curve = envelopes[active_plant].graph_nodes[graph_active_node].curve .. 'deg'
    ...
    elseif menu_status == false then 
      ...
      if label_obj[2] then
        ...
        if label_obj[2] == 1 then
          cursor_start = string_cut(label, 1, open_bracket_loc)
          cursor_start_x = screen.get_text_size(cursor_start)
          cursor_end_x = string_cut(label, 1, backslash_loc-1) 
          cursor_width = screen.get_text_size(cursor_end_x)  - cursor_start_x
          screen.move(cursor_start_x,9)
          screen.line_rel(cursor_width,0)
        elseif label_obj[2] == 2 then
          cursor_start = string_cut(label, 1, open_paren_loc)
          cursor_start_x = screen.get_text_size(cursor_start)
          cursor_end_x = string_cut(label, 1, plus_min_loc-1) 
          cursor_width = screen.get_text_size(cursor_end_x)  - cursor_start_x
          screen.move(cursor_start_x,9)
          screen.line_rel(cursor_width+2,0)
        elseif label_obj[2] == 3 then
          cursor_start = string_cut(label, 1, plus_min_loc)
          cursor_start_x = screen.get_text_size(cursor_start)
          cursor_end_x = string_cut(label, 1, close_paren_loc-1) 
          cursor_width = screen.get_text_size(cursor_end_x)  - cursor_start_x
          screen.move(cursor_start_x,9)
          screen.line_rel(cursor_width+2,0)
        end
  ...
  -- navigation marks
  screen.level(0)
  screen.move(0,math.ceil((pages.index-1)/num_pages*10))
  screen.rect_fill(2,2)
  -- --screen.update()
end
