...
modify.get_clipped_instruction = function(instruction)
  modify.max_visible_chars = 23 - #modify.nav_labels[modify.active_control]
  local cursor_index = modify.instruction_cursor_indices[active_plant][modify.active_control]
  ...
  letter_location.x, letter_location.y = screen.get_text_size(letters_left_of_cursor)
  ...
  local clipped_sentence_len = screen.get_text_size(clipped_sentence)
  local last_letter = string_cut(clipped_sentence,#clipped_sentence,#clipped_sentence)
  local last_letter_len = screen.get_text_size(last_letter)
...
modify.set_cursor = function()
  ...
  if modify.active_control <= (modify.num_rulesets * 2) + 1 then
    if ((modify.active_control - 1)%2) ~= 1 then
      ...
      local cursor_location = screen.get_text_size(label)+4+instruction_details[2]
      screen.text(label .. instructions)
      if show_instructions == false then
        screen.move(cursor_location, 3)
        screen.text('_')
      end
    end
  elseif modify.active_control - (modify.num_rulesets * 2) + 1 == 3 then
    -- axiom (axiom)
      ...
      -- cursor_location = cursor_location - screen.get_text_size(instructions) - 1
      local cursor_location = screen.get_text_size(label)+4+instruction_details[2]
      screen.text(label .. instructions)
      if show_instructions == false then
        screen.move(cursor_location, 3)
...
modify.draw_modify_nav = function()
  ...
  screen.move(2+(area_menu_width*(modify.active_field_menu_area-1)),10)
  screen.rect_fill(area_menu_width, 3)
  screen.level(4)
  for i=1, 11,1
  do
    if i < 11 then
      screen.move(2+(area_menu_width*(i-1)),10)
      screen.rect_fill(1, 3)
    else
      screen.move(2+(area_menu_width*(i-1))-1,10)
      screen.rect_fill(1, 3)
    end
  end
end
