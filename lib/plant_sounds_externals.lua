...
function plant_sounds_externals:new(active_notes)
  ...
  pse.midi_note_off = function(delay, note_num, channel, plant_id, note_location)
    ...
    if active_midi_notes and active_midi_notes[note_num] then
      active_midi_notes[note_num]=nil
    end
    -- if note_location <= #active_notes then
      -- active_midi_notes[note_to_play]=nil
    -- else
  ...
  pse.note_on = function(plant_id, note_to_play, pitch_frequency, beat_frequency, envelope_time_remaining, note_source, velocity)
    ...
    local total_nodes = plant_id == 1  and params:get("num_plow1_controls") or params:get("num_plow2_controls")
    local envelope_length = envelopes[plant_id].graph_nodes[total_nodes].time
    -- check for velocity
    ...
    if (note_source == "flora" and (output_midi == 2 or output_midi == 3)) or
      ...
      local max_envelope_level = 0
      for i=1, total_nodes do
        local node_level = envelopes[plant_id].graph_nodes[i].level
        if node_level > max_envelope_level then
          max_envelope_level = node_level
        end
      end
      local level = velocity * (max_envelope_level/MAX_AMPLITUDE) < 1 and velocity * (max_envelope_level/MAX_AMPLITUDE) or 1
      level = math.floor(util.linlin(0,1,0,127,level))
      ...
      if buchla208c then
        ...
        midi_out_device:cc(5, 10, 2)
        midi_out_device:cc(5, 10, 3)
      end
      
      table.insert(active_notes, note_to_play)
      -- Note off timeout
        -- print("envelope_length,level",envelope_length,level)
    ...
    if envelope_length > 1/beat_frequency then
      ...
      if time_remaining > 1/beat_frequency then
        ...
        if #_seamstress.clock.threads < 50 then
          clock.run(pse.note_on, plant_id, note_to_play, pitch_frequency, beat_frequency, time_remaining, note_source)
        end
      end
    end

      if active_midi_notes==nil then active_midi_notes={} end
      active_midi_notes[note_to_play]=note_to_play
    end
