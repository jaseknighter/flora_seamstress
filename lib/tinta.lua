...
function t.tintabulate()
  ...
  if t.note_pre_oct and t.note_pre_oct+chord_note < 1 then return end
  local t_note,t_note_pre
  
  if params:get("tin_dancing_notes") == 2 and t.note_pre_oct then
...
local main_screen_key = screen.key

function screen.key(character, modifiers, is_repeat, state)
  main_screen_key(character, modifiers, is_repeat, state)
  if pages.index==6 and character.name==nil and state == 1 then
    if shift_pressed then
      t.str = t.str .. kc.shifted[character]
    else
  ...
  local code = character.name == nil and character or character.name
