...
-- nav keys
--------------------------------
params:add_separator("nav keys")

params:add{type = "option", id = "e1", name = "e1",
  options = ENCODER_KEYVALS,
  default = 1,
  action = function()
    e1_key = ENCODER_KEYVALS[params:get('e1')]
end}

params:add{type = "option", id = "e2", name = "e2",
  options = ENCODER_KEYVALS,
  default = 2,
  action = function()
    e2_key = ENCODER_KEYVALS[params:get('e2')]
end}

params:add{type = "option", id = "e3", name = "e3",
  options = ENCODER_KEYVALS,
  default = 3,
  action = function()
    e3_key = ENCODER_KEYVALS[params:get('e3')]
end}
...
-- midi
  ...
  params:add{
    type = "option", id = "midi_device", name = "device", options = devices, 
  ...
  params:add{
    type = "option", id = "midi_out_device", name = "device", options = devices,
    min = 1, max = 16, default = 1,
    action = function(value) 
      if initializing == false and device_switching == false then 
        clock.run(all_midi_notes_off_del,0.5,midi_out_device) 
      end
      -- if #active_midi_notes>0 then all_midi_notes_off() end
...
params:add_group("just friends",2)
  ...
  -- from: https://github.com/ryleelyman/seamstress/blob/main/lua/core/clock.lua
...
--------------------------------
  ...
  params:hide("tin_env")
  params:hide("tin_env_morph")
  params:hide("tin_env_morph_style")
  --------------------------------
