      ...
      if sx_distance <= 1 or prev_sy == sy then
        ...
        for sample_x = prev_sx + 1, sx - 1 do
          ...
          if curve == "exp" then
            ...
            sy_section = util.linlin(0, 1, prev_py, py, a - (a * (grow^sample_x_progress)))
          end
  ...
  for i = 1, #self._points do
    ...
    if self._points[i].highlight then
      if sy < self.origin_sy then
        screen.move(sx - 1, sy)
        screen.rect(3, math.max(1, self.origin_sy - sy + 1))
      else
        screen.move(sx - 1, self.origin_sy)
        screen.rect(3, math.max(1, sy - self.origin_sy + 1))
      end
      ...
      elseif sy < self.origin_sy then
        screen.move(sx - 0.5, sy + 0.5)
        screen.rect(2, math.max(0, self.origin_sy - sy))
        --screen.stroke()
      else
        screen.move(sx - 0.5, self.origin_sy + 0.5)
        screen.rect(2, math.max(0, sy - self.origin_sy))
        --screen.stroke()
      end
    end
...
local function draw_lines(self)
  ...
  for l = 1, #self._lines do
    ...
    for i = 2, #self._lines[l] do
      ...
      screen.move(self._lines[l][i].x + 0.5, self._lines[l][i].y + 0.5)
    end
    --screen.stroke()
  end
...
local function draw_spline(self)
  ...
  for i = 1, #self._spline - 4, 4 do
    ...
    print("curve",self._spline[i + 1].x, self._spline[i + 1].y, self._spline[i + 2].x, self._spline[i + 2].y, self._spline[i + 3].x, self._spline[i + 3].y)
