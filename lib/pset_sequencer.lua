...
local function set_pset_seq_timer()
  ...
  pset_seq_timer = metro.init(function() 
    if params:get("pset_seq_enabled") == 2 then
      ...
      local first_value = params:get("pset_exclusion_first")
      local last_value = params:get("pset_exclusion_last")
      local new_pset_id
      pset_seq_ticks = pset_seq_ticks + 1
      if pset_seq_ticks == ticks_per_seq_cycle then
        pset_seq_ticks = 1
        local num_psets = last_value - first_value + 1
        -- local num_psets = pset_seq.get_num_psets()
        local current_pset = params:get("load_pset")
        local mode = params:get("pset_seq_mode")
        if mode == 1 then
          new_pset_id = current_pset < last_value and current_pset + 1 or first_value
        elseif mode == 2 then
          if first_value == last_value then
            new_pset_id = first_value
          elseif pset_seq_direction == "up" then
            new_pset_id = current_pset < last_value and current_pset + 1 or current_pset - 1
            pset_seq_direction = current_pset < last_value and pset_seq_direction or "down"
          else
            new_pset_id = current_pset > first_value and current_pset - 1 or current_pset + 1
            pset_seq_direction =  current_pset > first_value and pset_seq_direction or "up"
          end
        elseif mode == 3 then
          new_pset_id = math.random(1,num_psets) + first_value - 1
        end
...
function set_num_psets()
  local dir = util.scandir (pset_seq.pset_path)
...
pset_seq.init = function (pset_exclusion_tables, pset_exclusion_table_labels)
  ...
  -- pset_seq.pset_path = _path.data .. norns.state.name .. "/"
  pset_seq.pset_path = path.seamstress .. "/data/" .. seamstress.state.name
  ...
  params:add_number("load_pset", "load pset", 1, pset_seq.get_num_psets(),1,nil, nil, false, false)
  ...
  function set_pset_exclusion_first(val)
    ...
    elseif first_value > first.max then 
  ...
  function set_pset_exclusion_last(val)
    ...
    local last_value = params:get("pset_exclusion_last")
    if last_value == 0 or last_value > last.max then
  ...
  if pset_seq.pset_path == "/flora" then
